...
class ZephyrusPaths:
    ...
    def from_config(script_dir: Path) -> "ZephyrusPaths":
        ...
        vector_store_dir = ZephyrusPaths._resolve_path(
            config, "vector_store_dir", project_root / "vector_store"
        )

        json_log_file = ZephyrusPaths._resolve_path(
            config, "raw_log_path", log_dir / "zephyrus_log.json"
        )
        correction_summaries_file = ZephyrusPaths._resolve_path(
            config, "correction_summaries_path", log_dir / "correction_summaries.json"
        )
        ...
        faiss_index_path = ZephyrusPaths._resolve_path(
            config, "faiss_index_path", vector_store_dir / "summary_index.faiss"
        )
        faiss_metadata_path = ZephyrusPaths._resolve_path(
            config, "faiss_metadata_path", vector_store_dir / "summary_metadata.pkl"
        )
        raw_log_index_path = ZephyrusPaths._resolve_path(
            config, "raw_log_index_path", vector_store_dir / "raw_index.faiss"
        )
        raw_log_metadata_path = ZephyrusPaths._resolve_path(
            config, "raw_log_metadata_path", vector_store_dir / "raw_metadata.pkl"
        )
