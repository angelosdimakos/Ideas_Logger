...
def generate_testing_insights(audit: Dict[str, Any]) -> List[str]:
    ...
    missing = sum(
        1 for d in audit.values() for v in d.get("testing", {}).values() if v.get("tested") is False
    )
    if total == 0:
        return [
            "### 🧪 Testing Insight",
...
def generate_quality_insights(audit: Dict[str, Any]) -> List[str]:
    ...
    for data in audit.values():
        ...
        pass_count = 0
        # collect errors
        for issue in q.get("flake8", {}).get("issues", []):
            flake8[issue.get("code")] += 1
        for msg in q.get("pydocstyle", {}).get("issues", []):
            pydoc[msg] += 1
        ...
        if "flake8" in q:
            checks += 1
            pass_count += len(q["flake8"].get("issues", [])) == 0
        if "black" in q:
            checks += 1
            pass_count += not q["black"].get("needs_formatting", False)
        if "mypy" in q:
            checks += 1
            pass_count += len(q["mypy"].get("errors", [])) == 0
        if "pydocstyle" in q:
            checks += 1
            pass_count += len(q["pydocstyle"].get("issues", [])) == 0
        if "coverage" in q:
            checks += 1
            pass_count += q["coverage"].get("percent", 0) >= 80
        score_sum += pass_count / checks * 100 if checks else 0
    avg_score = score_sum / total_files if total_files else 0
    bar = render_bar(avg_score)
    emoji = risk_emoji(avg_score)
    parts = [
    ...
    for code, ct in flake8.most_common(3):
        parts.append(f"  - `{code}`: {ct}×")
    parts.append("- **Top MyPy Issues:**")
    for code, ct in mypy.most_common(3):
        parts.append(f"  - `{code}`: {ct}×")
    parts.append("")
    return parts
