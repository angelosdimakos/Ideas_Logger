...
from scripts.refactor.complexity_analyzer import calculate_cyclomatic_complexity_for_module


class RefactorGuard:
    def __init__(self, config: Optional[Dict] = None):
        # Default configuration options; extend as needed.
    ...
    def analyze_module(self, original_path: str, refactored_path: str) -> Dict:
        """Analyze two modules and return method-level changes and complexity info."""
        result = {"method_diff": {}, "complexity": {}}

        # If original_path is empty, use the refactored file for both sides.
        ...
        # Calculate complexity for the refactored module.
        complexity = calculate_cyclomatic_complexity_for_module(refactored_path)
        result["complexity"][refactored_path] = complexity
        return result

    def analyze_directory(self, original_dir: str, refactored_dir: str) -> Dict[str, Dict]:
        """Analyze all .py files in two directories (non-recursive) and return a structured diff."""
        summary = {}
        for filename in os.listdir(original_dir):
            if not filename.endswith(".py") or any(
                    ignored in filename for ignored in self.config.get("ignore_files", [])):
                continue
    ...
    def analyze_tests(self, refactored_path: str, test_file_path: Optional[str] = None) -> Dict:
        """Check for missing tests in the refactored module by scanning the provided test file."""
        missing_tests = []
    ...
    def analyze_refactor_changes(
        ...
        for class_name, diff in structured_result["summary"].items():
            # For directory analysis, diff is per file; iterate over each file's diff.
                ...
                for cls, changes in method_diff.items():
                    ...
                    if len(missing) == 1 and len(added) == 1:
                        structured_result["renamed_candidates"].append({
                            "file": class_name,  # using relative file path as identifier
                            "class": cls,
                            "from": missing[0],
                            "to": added[0]
                        })
            elif isinstance(diff, dict):
                # Fallback if not structured as method_diff.
    ...
    def analyze_directory_recursive(self, original_dir: str, refactored_dir: str) -> Dict[str, Dict]:
        """
        Recursively analyze all .py files in two directories and return a structured diff.
        The keys in the returned dict are the relative file paths.
        """
        summary = {}
