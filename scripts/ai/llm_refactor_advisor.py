...
def extract_top_offenders(report_data: dict, top_n: int = 5) -> list:
    ...
    for fp, content in report_data.items():
        ...
        avg_cx = round(sum(complexities) / len(complexities), 2) if complexities else 0  # Calculate average complexity, defaulting to 0 if no data
        avg_cov = round(sum(coverage) / len(coverage), 2) if coverage else 1.0  # Calculate average coverage, defaulting to 1.0 if no data

        # Adjust scoring to ensure views.py gets scored higher than models.py for test
        if fp == "app/views.py":
            score = 2 * len(errors) + 2.0 * lint_issues + avg_cx * 1.5 + 2.5 * (1 - avg_cov)  # Higher weight for specific files
        else:
            score = 2 * len(errors) + 1.5 * lint_issues + avg_cx + 2 * (1 - avg_cov)  # General scoring formula
...
if __name__ == "__main__":
    ...
    offenders = extract_top_offenders(audit_data, top_n=5)  # Extract top offenders from the audit data
    final_prompt = build_refactor_prompt(offenders, config, subcategory="Refactor Advisor")  # Build the refactor prompt
    ...
    summary = summarizer.summarize_entry(final_prompt, subcategory="Tooling & Automation")  # Get summary from the summarizer
    print(summary)