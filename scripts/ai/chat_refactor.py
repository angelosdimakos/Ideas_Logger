...
def build_contextual_prompt(query: str, report_data: dict, config) -> str:
    ...
    """
    top_files = sorted(
        report_data.items(), key=lambda x: x[1].get("severity_score", 0), reverse=True
    )[
        :5
    ]  # Select top 5 files based on severity score
    ...
    for fp, content in top_files:
        ...
        avg_cx = (
            round(sum(cx) / len(cx), 2) if cx else 0
        )  # Calculate average complexity, defaulting to 0 if no data
        avg_cov = (
            round(sum(cov) / len(cov), 2) if cov else 1.0
...
if __name__ == "__main__":
    ...
    while True:
        ...
        prompt = build_contextual_prompt(
            query, report, config
        )  # Build prompt based on user query and report
        print("\n🧠 Mistral (via Ollama) is thinking...\n")
        summary = summarizer.summarize_entry(
            prompt, subcategory="Tooling & Automation"
