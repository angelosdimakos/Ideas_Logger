...
def base_indexer(temp_dir):
    """
    Fixture for BaseIndexer tests using mock SentenceTransformer.
    """
...
def test_build_index_success(base_indexer):
    ...
    metadata = [{"id": 1}, {"id": 2}]
    assert base_indexer.build_index(texts, metadata)
...
def test_save_index_failure(caplog, base_indexer, monkeypatch):
    ...
    with caplog.at_level(logging.ERROR):
        ...
        assert result is False
        assert "Failed to save FAISS index" in caplog.text
