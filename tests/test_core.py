...
def test_generate_summary_triggers_and_writes_summary(logger_core):
    ...
    logs = {
        date_str: {
            category: {
                subcat: [{"timestamp": f"{date_str} 10:00:00", "content": f"entry {i}"} for i in range(5)]
            }
        }
    }
...
def test_generate_summary_fallback(logger_core):
    ...
    date = "2025-03-22"
    logger_core.json_log_file.write_text(json.dumps({
        date: {
            "Fallback": {
                "Test": [{"timestamp": f"{date} 11:00:00", "content": f"log {i}"} for i in range(5)]
            }
        }
    }, indent=2))
